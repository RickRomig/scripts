...
main() {
	...
	check_file2 || die "02-bookworm2trixie.sh hasn't been run." "$E_FILENOTFOUND"
	if check_codename; then
		printf "%sDebian in-place upgrade was successful!%s\n" "$orange" "$normal"
		clean_up
		modernize_sources
	else
		printf "%sDebian in-place upgrade failed.%s\n" "$orange" "$normal" >&2
		printf "Backup the home directory and any important files, then install from the most current netinstall ISO.\n" >&2
